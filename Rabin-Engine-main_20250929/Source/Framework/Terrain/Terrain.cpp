...
bool Terrain::initialize()
{
    ...
    const fs::directory_iterator bdir(Serialization::bigMapsPath);
...
{
    if (currentMap >= normalMapSize) currentMap = -1;
    // move to the next map
    currentMap = (currentMap + 1) % normalMapSize;
    
    load_map(currentMap);
}

void Terrain::goto_big_next_map()
{
    // move to the next map
    currentMap = std::max(normalMapSize, currentMap+1) % mapData.size();
	if (currentMap == 0) currentMap = normalMapSize; // skip to the first big map if we wrap around
...
size_t Terrain::num_normal_map() const
{
