...
void PathTester::execute_speed_test()
{
    ...
    if (file)
    {
        ...
        file << std::setw(width) << "Iter #" << "Duration [microseconds]" << std::endl;
        std::string temp;
...
// this only for the big map
void PathTester::execute_big_map_test()
{
    terrain->goto_map(static_cast<unsigned int>(terrain->num_normal_map()));
    ...
    for (size_t i = 0; i < numIter; ++i)
    {
        ...
        for (const auto& [start, goal] : bigMapPaths)
        {
            agent->set_position(terrain->get_world_position(start));
            agent->path_to(terrain->get_world_position(goal), false);
        }
...
// this compare both small and big map
void PathTester::execute_all_speed_test()
{
    terrain->goto_map(1);
    ...
    const size_t numIter1 = 1000;

    using ms = std::chrono::microseconds;
    std::array<ms, numIter1> results1;
    ms::rep fastest1 = std::numeric_limits<ms::rep>().max();
    ms::rep total1 = 0;

    for (size_t i = 0; i < numIter1; ++i)
    {
        timer1.start();
        ...
        results1[i] = timer1.microseconds();

        if (results1[i].count() < fastest1)
        {
            fastest1 = results1[i].count();
        }
    ...
    if (file)
    {
        // the number 0.0044 is come by 40*40/800*800 wich roughly the size of the map
        float effective = float(fastest) * 0.0044f / float(fastest1);
        file << "Effectiveness: " << effective
            << " big/small" << std::endl;
        ...
        for (size_t i = 0; i < numIter1; ++i)
        {
            temp1 = std::to_string(i + 1) + ":";
            file << std::setw(width) << temp1 << results1[i].count() << std::endl;
        }
...
bool PathTester::load_tests()
{
    ...
    for (auto && entry : dir)
    {
        if (fs::is_regular_file(entry) == true)
        {
            ...
            if (path.filename() == "Speed_big.txt")
            {
                Serialization::deserialize(bigMapPaths, path);
            }
            else
            {
